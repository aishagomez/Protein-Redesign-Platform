...
c changed nmax in ALL subroutines for consistency with C dimensions
      PARAMETER(nmax=5000)     ! original nmax=5000
      PARAMETER(nmax2=10000)    ! original nmax2=10000
      ...
      common/sequ/ss1,ss2                                !interface with C
      common/trg/xtrg(nmax),ytrg(nmax),ztrg(nmax)        !interface with C
      common/tpl/xtpl(nmax),ytpl(nmax),ztpl(nmax)        !interface with C
      common/ram/n8_al,rmsd8_al,TMS,seq_id,TR,UR         !interface with C
      real TR(3),UR(9)                                   !interface with C
...
c         write(*,*)
c         write(*,*)'Brief instruction for running TM-align program:'
c         write(*,*)'(For detail: Zhang & Skolnick, Nucl. Acid. Res.',
c     &     ' 33: 2303, 2005)'
c         write(*,*)
c         write(*,*)'1. Align ''chain_1.pdb'' and ''chain_2.pdb'':'
c         write(*,*)'   >TMalign chain_1.pdb chain_2.pdb'
c         write(*,*)
c         write(*,*)'2. Ask TM-align to start with an alignment',
c     &        ' specified in fasta file ''align.txt'':'
c         write(*,*)'   >TMalign chain_1.pdb chain_2.pdb -i align.txt'
c         write(*,*)'   or to stick the alignment to ''align.txt'':'
c         write(*,*)'   >TMalign chain_1.pdb chain_2.pdb -I align.txt'
c         write(*,*)
c         write(*,*)'3. Output the superposition to ''TM.sup'', ',
c     &        '''TM.sup_all'' and ''TM.sup_atm'':'
c         write(*,*)'   >TMalign chain_1.pdb chain_2.pdb -o TM.sup'
c         write(*,*)'      To view superimposed C-alpha traces of',
c     &        ' aligned regions by rasmol:'
c         write(*,*)'        >rasmol -script TM.sup'
c         write(*,*)'      To view superimposed C-alpha traces of',
c     &        ' all regions:'
c         write(*,*)'        >rasmol -script TM.sup_all'
c         write(*,*)'      To view superimposed full-atom structures of',
c     &        ' aligned regions:'
c         write(*,*)'        >rasmol -script TM.sup_atm'
c         write(*,*)'      To view superimposed full-atom structures of',
c     &        ' all regions:'
c         write(*,*)'        >rasmol -script TM.sup_all_atm'
c         write(*,*)
c         write(*,*)'4. There are two TM-scores reported. You ',
c     &        'should use the one normalized by'
c         write(*,*)'   the length of the protein you ',
c     &        'are interested in.'
c         write(*,*)'   If you want TM-score normalized by the ',
c     &        'average length of two proteins:'
c         write(*,*)'      >TMalign chain_1.pdb chain_2.pdb -a'
c         write(*,*)'   or TM-score normalized by an ',
c     &        'assigned length (>L_min), e.g. 100 AA:'
c         write(*,*)'      >TMalign chain_1.pdb chain_2.pdb -L 100'
c         write(*,*)'   If you want TM-score scaled by an assigned d0,',
c     &        ' e.g. 5 A:'
c         write(*,*)'      >TMalign chain_1.pdb chain_2.pdb -d 5'
c         write(*,*)
c         write(*,*)'5. Output TM-align rotation matrix:'
c         write(*,*)'   >TMalign chain_1.pdb chain_2.pdb -m matrix.txt'
c         write(*,*)
c         goto 9999
c      endif
...
c      version='20130511'
c      if(fnam.eq.'-v')then
c         write(*,*)'TM-align Version ',version
c         goto 9999
c      endif
...
c      m_matrix=-1               !no output of matrix
      m_matrix=1                ! output matrix - interface with C
...
c      elseif(fnam.eq.'-a')then  !change superposed output but not the alignment
c         m_ave=1
c         i=i+1
c      elseif(fnam.eq.'-L')then  !change both L_all and d0
c         m_fix=1
c         i=i+1
c         call getarg(i,fnam)
c         read(fnam,*)L_fix
...
c         call getarg(i,fnam)
c         read(fnam,*)d0_fix
c      elseif(fnam.eq.'-i')then
c         m_alignment=1
c         i=i+1
c         call getarg(i,fnam)
c         falign=fnam
c      elseif(fnam.eq.'-I')then
c         m_alignment_stick=1
c         i=i+1
c         call getarg(i,fnam)
c         falign=fnam
c      elseif(fnam.eq.'-m')then
c         m_matrix=1
c         i=i+1
c         call getarg(i,fnam)
c         fmatrix=fnam
...
c      ntt=0
c      do while (.true.)
c         read(10,9001,end=1010) s
c         if(i.gt.0.and.s(1:3).eq.'TER')goto 1010
c         if(s(1:3).eq.'ATO')then
c***   
c            if(s(27:27).eq.' ')then
c               du1=' '
c            else
c               read(s(27:27),*)du1
c            endif
c            mk=1
c            if(s(17:17).ne.' ')then
c               read(s(13:16),*)du
c               read(s(23:26),*)i8
c               do i1=1,nres1(i8,ichar(du1))
c                  if(du.eq.atom1(i8,i1))then
c                     mk=-1
c                  endif
c               enddo
c            endif
...
c               read(s(13:16),*)du
c               read(s(23:26),*)i8
c               if(nres1(i8,ichar(du1)).lt.30)then
c                  nres1(i8,ichar(du1))=nres1(i8,ichar(du1))+1
c                  atom1(i8,nres1(i8,ichar(du1)))=du
c               endif
c***   
c               ntt=ntt+1
c               if(ntt.ge.90000)goto 1010
c               na1=na1+1
c               read(s,8999)du,ia1(na1),du,aa1(na1),du,ra1(na1),du,
c     &              ir1(na1),du,xa1(na1),ya1(na1),za1(na1)
c               ains1(na1)=du1
c               if(s(13:16).eq.'CA  '.or.s(13:16).eq.' CA '.or
c     &              .s(13:16).eq.'  CA')then
c                  i=i+1
c                  read(s,9000)du,ma1(i),du,aanam,du,mm1(i),du,
c     $                 xa(1,i,0),xa(2,i,0),xa(3,i,0)
c                  ins1(i)=du1
c                  do j=-1,20
c                     if(aanam.eq.aa(j))then
c                        seq1(i)=slc(j)
c                        goto 21
c                     endif
c                  enddo
...
c 21               continue
c                  ss1(i)=aanam
c                  if(i.ge.nmax)goto 1010
c               endif
c            endif
c         endif
c      enddo
c 1010 continue
c 8999 format(a6,I5,a1,A4,a1,A3,a2,I4,a4,3F8.3)
c 9000 format(a6,I5,a6,A3,A2,i4,A4,3F8.3)
c 9001 format(A100)
c      close(10)
c      nseq1=i
c^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
c      ntt=0
c      do while (.true.)
c         read(10,9001,end=1011) s
c         if(i.gt.0.and.s(1:3).eq.'TER')goto 1011
c         if(s(1:3).eq.'ATO')then
c***   
c            if(s(27:27).eq.' ')then
c               du1=' '
...
c               read(s(13:16),*)du
c               read(s(23:26),*)i8
c               do i1=1,nres2(i8,ichar(du1))
c                  if(du.eq.atom2(i8,i1))then
c                     mk=-1
c                  endif
c               enddo
c            endif
...
c               read(s(13:16),*)du
c               read(s(23:26),*)i8
c               if(nres2(i8,ichar(du1)).lt.30)then
c                  nres2(i8,ichar(du1))=nres2(i8,ichar(du1))+1
c                  atom2(i8,nres2(i8,ichar(du1)))=du
c               endif
c***   
c               ntt=ntt+1
c               if(ntt.ge.90000)goto 1011
c               na2=na2+1
c               read(s,8999)du,ia2(na2),du,aa2(na2),du,ra2(na2),du,
c     &              ir2(na2),du,xa2(na2),ya2(na2),za2(na2)
c               ains2(na2)=du1
c               if(s(13:16).eq.'CA  '.or.s(13:16).eq.' CA '.or.
c     &              s(13:16).eq.'  CA')then
c                  i=i+1
c                  read(s,9000)du,ma2(i),du,aanam,du,mm2(i),du,
c     $                 xa(1,i,1),xa(2,i,1),xa(3,i,1)
c                  ins2(i)=du1
c                  do j=-1,20
c                     if(aanam.eq.aa(j))then
c                        seq2(i)=slc(j)
...
c      nseq2=i
c^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
ccccccccc assign coordinates for first molecule:
      do i=1,nseq1
         xa(1,i,0)=xtrg(i)
         xa(2,i,0)=ytrg(i)
         xa(3,i,0)=ztrg(i)
...
ccccccccc assign coordinates for second molecule:
      do i=1,nseq2
         xa(1,i,1)=xtpl(i)
         xa(2,i,1)=ytpl(i)
         xa(3,i,1)=ztpl(i)
...
c      if(m_alignment.eq.1.or.m_alignment_stick.eq.1)then
c         open(unit=10,file=falign,status='old')
c         n_p=0
c         do while (.true.)
c            read(10,9002,end=1012)s1
cc            write(*,*)'s1=',trim(s1)
c            if(s1(1:1).eq.">")then
c               n_p=n_p+1
c               sequence(n_p)=''
c               if(n_p.gt.2)goto 1012
c            else
c               if(n_p.gt.0)then
c                  sequence(n_p)=sequence(n_p)(1:len_trim(sequence(n_p)))
c     &                 //s1(1:len_trim(s1))
cc                  write(*,*)n_p,trim(sequence(n_p))
c               endif
c            endif
c         enddo
c 1012    continue
c         close(10)
cc         write(*,*)trim(sequence(1))
cc         write(*,*)trim(sequence(2))
c         if(n_p.lt.2)then
c            write(*,*)'ERROR: FASTA format is wrong, two proteins',
c     &           ' should be included'
c            stop
c         endif
c         if(len_trim(sequence(1)).ne.len_trim(sequence(2)))then
c            write(*,*)'Warning: FASTA format may be wrong, the',
c     &           ' length in alignment should be equal. But we',
c     &           ' run it anyway'
c         endif
c      endif
c 9002 format(A5000)
...
c      write(*,*)'*                        TM-align (Version ',version,
c     &     ')                     *'
c      write(*,*)'* An algorithm for protein structure alignment and co',
c     &     'mparison            *'
c      write(*,*)'* Based on statistics:                               ',
c     &     '                    *'
c      write(*,*)'*       0.0 < TM-score < 0.30, random structural simi',
c     &     'larity              *'
c      write(*,*)'*       0.5 < TM-score < 1.00, in about the same fold',
c     &     '                    *'
c      write(*,*)'* Reference: Y Zhang and J Skolnick, Nucl Acids Res 3',
c     &     '3, 2302-9 (2005)    *'
c      write(*,*)'* Please email your comments and suggestions to: zhng',
c     &     '@umich.edu          *'
c      write(*,*)'*****************************************************',
c     &     '*********************'
c      write(*,*)
c      write(*,101)pdb(1)
c 101  format('Name of Chain_1: ',A50)
c      write(*,102)pdb(2)
c 102  format('Name of Chain_2: ',A50)
c      write(*,103)nseq1
c 103  format('Length of Chain_1: ',I4,' residues')
c      write(*,201)nseq2
c 201  format('Length of Chain_2: ',I4,' residues')
c      
c      if(m_alignment.eq.1.or.m_alignment_stick.eq.1)then
c         write(*,72)TM_ali,L_ali,rmsd_ali
c 72      format('User-specified initial alignment: TM/Lali/rmsd= ',
c     &        f7.5,', ',I4,', ',f6.3)
c      endif
c      
c      write(*,*)

      rmsd8_al=rmsd   !interface with C
      TMS=TM2         !interface with C - TM-score normalized by Chain_2

c      write(*,203)n8_al,rmsd,seq_id
c 203  format('Aligned length= ',I4,', RMSD= ',f6.2,
c     &     ', Seq_ID=n_identical/n_aligned= ',f5.3)
c      write(*,204)TM1
c 204  format('TM-score= ',f7.5,' (if normalized by length of Chain_1)')
c      write(*,205)TM2
c 205  format('TM-score= ',f7.5,' (if normalized by length of Chain_2)')
c      if(m_ave.eq.1)then
c         write(*,206)TM12,(nseq1+nseq2)/2.0
c 206     format('TM-score= ',f7.5,
c     &        ' (if normalized by average length of chains =',f6.1,')')
c      endif
c      if(m_fix.eq.1)then
c         write(*,207)TML,L_fix
c 207     format('TM-score= ',f7.5,
c     &        ' (if scaled by user-specified L=',I4,')')
c      endif
c      if(m_d0.eq.1)then
c         write(*,208)TMfix,d0_fix
c 208     format('TM-score= ',f7.5,
c     &        ' (if scaled by user-specified d0=',f4.1,')')
c      endif
c      write(*,210)
c 210  format('(You should use TM-score normalized by length',
c     &     ' of the reference protein)')
c      write(*,*)
...
********* extract rotation matrix based on TMscore8 ------------>
      if(m_matrix.eq.1.or.m_out.eq.1)then !we need to extract rotation matrix
         ...
         enddo
         if(L.le.3)then
c            write(*,*)'Aligned length is too short,',
c     &           ' no matrix outout'
            goto 211
         endif
...
c         open(unit=1,file=fmatrix,status='unknown')
c         write(1,*)'-------- Rotation matrix to rotate Chain_1 to ',
c     &        'Chain_2 ------'
c         write(1,*)'m          t(m)         u(m,1)         u(m,2) ',
c     &        '        u(m,3)'
         jr=0               !interface with C
         do i=1,3
c            write(1,209)i,t(i),u(i,1),u(i,2),u(i,3)
            TR(i)=t(i)      !interface with C    
            UR(jr+1)=u(i,1) !interface with C
            UR(jr+2)=u(i,2) !interface with C
            UR(jr+3)=u(i,3) !interface with C
            jr=jr+3         !interface with C        
         enddo

c         write(1,*)'Code for rotating Chain_1 from (x,y,z) to (X,Y,Z):'
c         write(1,*)'   do i=1,L'
c         write(1,*)'     X(i)=t(1)+u(1,1)*x(i)+u(1,2)*y(i)+u(1,3)*z(i)'
c         write(1,*)'     Y(i)=t(2)+u(2,1)*x(i)+u(2,2)*y(i)+u(2,3)*z(i)'
c         write(1,*)'     Z(i)=t(3)+u(3,1)*x(i)+u(3,2)*y(i)+u(3,3)*z(i)'
c         write(1,*)'   enddo'
c         write(1,*)
c         close(1)
c 209     format(I2,f18.10,f15.10,f15.10,f15.10)
      endif
      
********* output superposition ******************************
c      if(m_out.eq.1)then
cc     11, output superimpostion in the aligned regions ------------->
c 1236    format('ATOM  ',i5,'  CA  ',A3,' A',I4,A1,3X,3F8.3)
c 1237    format('ATOM  ',i5,'  CA  ',A3,' B',I4,A1,3X,3F8.3)
c 1238    format('TER')
c 1239    format('CONECT',I5,I5)
c 900     format(A)
c 902     format('select ',I4,':A,',I4,':B')
c 903     format('REMARK TM-align Version ',A8,'')
c 104     format('REMARK Chain 1:',A10,'  Size=',I4)
c 105     format('REMARK Chain 2:',A10,'  Size=',I4,
c     &        ' (TM-score is normalized by ',I4,', d0=',f6.2,')')
c 106     format('REMARK Aligned length=',I4,', RMSD=',f6.2,
c     &        ', TM-score=',f7.5,', ID=',f5.3)
c         OPEN(unit=7,file=outname,status='unknown') !TM.sup=pdb1.aln + pdb2.aln
***   script:
c         write(7,900)'load inline'
c         write(7,900)'select *A'
c         write(7,900)'wireframe .45'
c         write(7,900)'select *B'
c         write(7,900)'wireframe .20'
c         write(7,900)'select all'
c         write(7,900)'color white'
c         do i=1,n8_al
c            dis2=sqrt((xtm1(i)-xtm2(i))**2+
c     &           (ytm1(i)-ytm2(i))**2+(ztm1(i)-ztm2(i))**2)
c            if(dis2.le.d0_out)then
c               write(7,902)mm1(m1(i)),mm2(m2(i))
c               write(7,900)'color red'
c            endif
c         enddo
c         write(7,900)'select all'
c         write(7,900)'exit'
c         write(7,903)version
c         write(7,104)pdb(1),nseq1
c         write(7,105)pdb(2),nseq2,int(anseq),d0
c         write(7,106)n8_al,rmsd,TM2,seq_id
c***   chain1:
c         do i=1,n8_al
c            write(7,1236)m1(i),ss1(m1(i)),mm1(m1(i)),ins1(m1(i)),
c     &           xtm1(i),ytm1(i),ztm1(i)
c         enddo
c         write(7,1238)          !TER
c         do i=2,n8_al
c            write(7,1239)m1(i-1),m1(i) !connect atoms
c         enddo
c***   chain2:
c         do i=1,n8_al
c            write(7,1237)5000+m2(i),ss2(m2(i)),mm2(m2(i)),ins2(m2(i)),
c     $           xtm2(i),ytm2(i),ztm2(i)
c         enddo
c         write(7,1238)
c         do i=2,n8_al
c            write(7,1239)5000+m2(i-1),5000+m2(i)
c         enddo
c         close(7)
cc     22, output CA-trace of whole chain in 'TM.sup_all' -------->
c         outnameall=outname(1:len_trim(outname))//'_all'
c         OPEN(unit=8,file=outnameall,status='unknown') !pdb1.aln + pdb2.aln
c***   script:
c         write(8,900)'load inline'
c         write(8,900)'select *A'
c         write(8,900)'wireframe .45'
c         write(8,900)'select none'
c         write(8,900)'select *B'
c         write(8,900)'wireframe .20'
c         write(8,900)'color white'
c         do i=1,n8_al
c            dis2=sqrt((xtm1(i)-xtm2(i))**2+
c     &           (ytm1(i)-ytm2(i))**2+(ztm1(i)-ztm2(i))**2)
c            if(dis2.le.d0_out)then
c               write(8,902)mm1(m1(i)),mm2(m2(i)) !select residue
c               write(8,900)'color red'
c            endif
c         enddo
c         write(8,900)'select all'
c         write(8,900)'exit'
c         write(8,903)version
c         write(8,104)pdb(1),nseq1
c         write(8,105)pdb(2),nseq2,int(anseq),d0
c         write(8,106)n8_al,rmsd,TM2,seq_id
c***   chain1:
c         do i=1,nseq1
...
c            write(8,1236)i,ss1(i),mm1(i),ins1(i),ax,ay,az
c         enddo
c         write(8,1238)          !TER
c         do i=2,nseq1
c            write(8,1239)i-1,i !CONECT atom numbers
c         enddo
c***   chain2:
c         do i=1,nseq2
c            write(8,1237)5000+i,ss2(i),mm2(i),ins2(i),
c     $           xa(1,i,1),xa(2,i,1),xa(3,i,1)
c         enddo
c         write(8,1238)
c         do i=2,nseq2
c            write(8,1239)5000+i-1,5000+i
c         enddo
c         close(8)
cc     33, output full-atomic structure of whole chain in 'TM.sup_atm' -------->
c         outnameall=outname(1:len_trim(outname))//'_atm'
c         OPEN(unit=8,file=outnameall,status='unknown') !pdb1.aln + pdb2.aln
c***   script:
c         write(8,900)'load inline'
c         write(8,900)'select *A'
c         write(8,900)'color blue'
c         write(8,900)'select *B'
c         write(8,900)'color red'
c         write(8,900)'select all'
c         write(8,900)'cartoon'
c         write(8,900)'exit'
c         write(8,903)version
c         write(8,104)pdb(1),nseq1
c         write(8,105)pdb(2),nseq2,int(anseq),d0
...
c         do i=1,na1
c            do j=1,n8_al
c               if(ir1(i).eq.mm1(m1(j)))then !aligned residues
c                  if(ains1(i).eq.ins1(m1(j)))then
c                     ax=t(1)+u(1,1)*xa1(i)+u(1,2)*ya1(i)+u(1,3)*za1(i)
c                     ay=t(2)+u(2,1)*xa1(i)+u(2,2)*ya1(i)+u(2,3)*za1(i)
c                     az=t(3)+u(3,1)*xa1(i)+u(3,2)*ya1(i)+u(3,3)*za1(i)
c                     write(8,8888)ia1(i),aa1(i),ra1(i),ir1(i),ains1(i),
c     &                    ax,ay,az
c                  endif
c               endif
c            enddo
...
c         do i=1,na2
c            do j=1,n8_al
c               if(ir2(i).eq.mm2(m2(j)))then !aligned residues
c                  if(ains2(i).eq.ins2(m2(j)))then
c                     write(8,8889)ia2(i),aa2(i),ra2(i),ir2(i),ains2(i),
c     &                    xa2(i),ya2(i),za2(i)
c                  endif
c               endif
c            enddo
...
c         write(8,1238)          !TER
c         close(8)
cc     44, output full-atomic structure of whole chain in 'TM.sup_all_atm' -------->
c         outnameall=outname(1:len_trim(outname))//'_all_atm'
c         OPEN(unit=8,file=outnameall,status='unknown') !pdb1.aln + pdb2.aln
c***   script:
c         write(8,900)'load inline'
c         write(8,900)'select *A'
...
c         write(8,1238)          !TER
c         close(8)
c 8888    format('ATOM  ',I5,1x,A4,1x,A3,' A',I4,A1,3x,3F8.3)
c 8889    format('ATOM  ',I5,1x,A4,1x,A3,' B',I4,A1,3x,3F8.3)
c      endif
*^^^^^^^^^^^^^^^^^^ output finished ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
c      i2_old=1
c      do i=1,n8_al
c         do j=i1_old,m1(i)-1
c            ii=ii+1
c            aseq1(ii)=seq1(j)
c            aseq2(ii)='-'
c            aseq3(ii)=' '
c         enddo
c         do j=i2_old,m2(i)-1
c            ii=ii+1
c            aseq1(ii)='-'
c            aseq2(ii)=seq2(j)
c            aseq3(ii)=' '
c         enddo
c         ii=ii+1
c         aseq1(ii)=seq1(m1(i))
c         aseq2(ii)=seq2(m2(i))
c         dis2=sqrt((xtm1(i)-xtm2(i))**2+
c     &     (ytm1(i)-ytm2(i))**2+(ztm1(i)-ztm2(i))**2)
c         if(dis2.le.d0_out)then
c           aseq3(ii)=':'
...
c      enddo
c      write(*,50)d0_out
c 50   format('(":" denotes aligned residue pairs of d < ',f3.1,
c     &     ' A, "." denotes other aligned residues)')
c      write(*,10)(aseq1(i),i=1,ii)
c      write(*,10)(aseq3(i),i=1,ii)
c      write(*,10)(aseq2(i),i=1,ii)
c 10   format(10000A1)
c      write(*,*)
